...
if(isset($_POST['btn_add'])){
    ...
    if($num_rows == 0){
        ...
        $query = mysqli_query($con,"INSERT INTO tblclearance (clearanceNo,fullname,purpose,orNo,samount,dateRecorded,recordedBy,status) 
            values ('$txt_cnum','$txt_name','$txt_purpose', '$txt_ornum', '$txt_amount', '$date', '".$_SESSION['username']."','Approved')") or die('Error: ' . mysqli_error($con));
        }
        else{
        $query = mysqli_query($con,"INSERT INTO tblclearance (clearanceNo,fullname,purpose,orNo,samount,dateRecorded,recordedBy,status) 
            values ('$txt_cnum','$txt_name', '$txt_findings','$txt_purpose', '$txt_ornum', '$txt_amount', '$date', '".$_SESSION['username']."','New')") or die('Error: ' . mysqli_error($con));
        }
        if($query == true)
        {
            $_SESSION['added'] = 1;
    ...
    if($num_row > 0)
    {
        ...
        while($row = mysqli_fetch_array($chk)){
            ...
            if($row['lengthofstay'] < 6){
                ...
                $txt_purpose = $_POST['txt_purpose'];
                $date = date('Y-m-d');
                $reqquery = mysqli_query($con,"INSERT INTO tblclearance (clearanceNo,fullname,purpose,orNo,samount,dateRecorded,recordedBy,status) 
